...
cb = Couchbase.connect(bucket='beer-sample', password='beer')
...
if not 'by_name' in beer_design.value['views']:
    ...
    cb.design_create("beer", beer_design.value, syncwait=5, use_devmode=False)
...
try:
    ...
    if not 'by_name' in b_design.value['views']:
        ...
        cb.design_create("brewery", beer_design.value, syncwait=5, use_devmode=False)

except HTTPError:
    cb.design_create("brewery", breweries_design, use_devmode=False, syncwait=5)